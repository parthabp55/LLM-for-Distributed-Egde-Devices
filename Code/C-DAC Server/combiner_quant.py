...
#         model_path,
#         device_map="auto",  # Auto will default to CPU now
...
#     inputs = tokenizer(input_text, return_tensors="pt").to(device)  # Move input tensors to CPU
#     with torch.no_grad():
...
#     )
#     inputs = tokenizer(prompt, return_tensors="pt", truncation=True).to(device)  # Move input tensors to CPU
...
#     )
#     inputs = refiner_tokenizer(prompt, return_tensors="pt", truncation=True).to(device)  # Move input tensors to CPU
#     with torch.no_grad():
...
#     embedder = SentenceTransformer(cfg["embedder_model"], device=device)  # Ensure embedder runs on CPU
...
#     logger.info(f"Cosine Similarity → {np.mean(results['cosine']):.4f}")
#     logger.info(f"Confidence Score → {np.mean(results['confidence']):.4f}")
#     logger.info(f"Tokens Per Second → {np.mean(results['tps']):.2f}")
